...
def build_graph(data_reader, conf_dict, word, postag, p_word, seq_len):
    # Placeholders
    ...
    p_embedding = tf.nn.embedding_lookup(tf.convert_to_tensor(_word_embedding, np.float32), p_word)
    ...
    with tf.variable_scope("p_fc"):
        p_inputs = full_connect(tf.reshape(p_embedding,[-1,conf_dict['word_dim']]), hidden_dim)

    word_rnn_inputs = word_inputs + postag_inputs + p_inputs
    ...
    word_rnn_outputs = tf.reshape(word_rnn_outputs, [-1, hidden_dim * 2])
    with tf.variable_scope("out_weights"):
        weights_out = tf.get_variable(name="output_w", dtype=tf.float32, shape=[hidden_dim * 2, data_reader.get_dict_size('so_label_dict')])
        biases_out = tf.get_variable(name="output_bias", dtype=tf.float32, shape=[data_reader.get_dict_size('so_label_dict')])

    dense = tf.matmul(word_rnn_outputs, weights_out) + biases_out
    dense = tf.reshape(dense, [-1, conf_dict['max_len'], data_reader.get_dict_size('so_label_dict')])
    return dense
